...
describe('Switch', () => {
  test('create', () => {
    ...
    expect(vm.$el.style.getPropertyValue('--ef-switch-on-color')).toEqual(
      '#0f0'
    );
    expect(vm.$el.style.getPropertyValue('--ef-switch-off-color')).toEqual(
      '#f00'
    );

    expect(vm.$el.classList.contains('is-checked')).false;
    const coreEl = vm.$el.querySelector('.ef-switch__core');
    expect(coreEl.style.width).toEqual('100px');
    const leftLabelWrapper = wrapper.find('.ef-switch__label--left span');
    expect(leftLabelWrapper.text()).toEqual('off');
  });

  test('size', () => {
    const wrapper = mount(() => <Switch size="large" />);
    expect(wrapper.find('.ef-switch--large').exists()).toBe(true);
  });
  test('tabindex', () => {
    const wrapper = mount(() => <Switch tabindex="0" />);
    expect(wrapper.find('.ef-switch__input').attributes().tabindex).toBe('0');
  });
  ...
  test('inline prompt', () => {
    ...
    const coreEl = vm.$el.querySelector('.ef-switch__core');
    expect(coreEl.style.width).toEqual('100px');
    const leftLabelWrapper = wrapper.find('.ef-switch__inner span');
  ...
  test('switch with icons', () => {
    const wrapper = mount(() => (
      <Switch
        activeIcon="i-line-md-cloud-loop"
        inactiveIcon="i-line-md-cloud-outline-loop"
      />
    ));

    expect(wrapper.find('.i-line-md-cloud-loop').exists()).toBe(true);
  });
  ...
  test('value correctly update', async () => {
    ...
    const coreWrapper = wrapper.find('.ef-switch__core');
  ...
  test('change event', async () => {
    ...
    const wrapper = mount(() => (
      <Switch v-model={value.value} onChange={handleChange} />
    ));
  ...
  describe('form item accessibility integration', () => {
    test('automatic id attachment', async () => {
      ...
      const formItemLabel = formItem.find('.ef-form-item__label');
      const switchInput = wrapper.find('.ef-switch__input');
