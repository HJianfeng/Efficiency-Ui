...
import type { InputAutoSize, InputInstance, InputProps } from '../input';
...
describe('Input.vue', () => {
  ...
  describe('test emoji', () => {
    test('el-input should minimize value between emoji length and maxLength', async () => {
      ...
      const elCount = wrapper.find('.ef-input-count__inner');
  ...
  test('resize', async () => {
    ...
    const wrapper = mount(() => (
      <Input type="textarea" resize={resize.value} />
    ));
  ...
  test('sets value on textarea / input type change', async () => {
    ...
    const wrapper = mount(() => (
      <Input type={type.value} v-model={val.value} />
    ));
    const vm = wrapper.vm;
  ...
  test('limit input and show word count', async () => {
    ...
    show.value = true;
    await nextTick();
    expect(inputElm1.querySelectorAll('.ef-input-count').length).toEqual(1);
  ...
  describe('Input Events', () => {
    ...
    test('event:clear', async () => {
      ...
      await input.trigger('focus');
      await nextTick();
      vm.$el.querySelector('.ef-input__clear').click();
      await nextTick();
  ...
  test('show-password icon', async () => {
    ...
    const icon = wrapper.find('.ef-input__password');
    expect(
      wrapper.vm.$el.querySelectorAll('.i-ph-eye-slash-light').length
    ).toEqual(1);
    await icon.trigger('click');
    expect(wrapper.vm.$el.querySelectorAll('.i-ph-eye-light').length).toEqual(
      1
    );
  });
});
