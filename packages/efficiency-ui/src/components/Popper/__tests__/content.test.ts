...
import EfContent from '../content.vue';
...
describe('<EfPopperContent />', () => {
  describe('with triggerRef provided', () => {
    const triggerKls = 'ef-popper__trigger';
    ...
    it('should mount the component correctly and set popperInstance correctly', async () => {
      ...
      expect(wrapper.classes()).toEqual(['ef-popper', 'is-dark']);
    ...
    it('should be able to be pure and themed', async () => {
      ...
      expect(wrapper.classes()).toEqual(['ef-popper', 'is-pure', 'is-custom']);
    });
    ...
    it('should be able to emit events', async () => {
      ...
      await wrapper.vm.$emit('mouseenter');
      expect(wrapper.emitted()).toHaveProperty('mouseenter');

      await wrapper.vm.$emit('mouseleave');
